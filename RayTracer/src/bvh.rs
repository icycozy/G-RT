...
impl BVHNode {
    ...
    pub fn new_recursive(objects: &mut Vec<Box<dyn Hittable>>, start: usize, end: usize) -> Self {
        ...
        let (left, right): (Box<dyn Hittable>, Box<dyn Hittable>);
        ...
        if object_span == 1 {
            ...
            left = Box::new(BVHNode::new_recursive(objects, start, mid));
            right = Box::new(BVHNode::new_recursive(objects, mid, end));
        }
