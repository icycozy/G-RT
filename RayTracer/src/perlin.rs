...
impl Perlin {
    ...
    pub fn new() -> Self {
        ...
        for i in 0..Self::POINT_COUNT {
            randvec[i] = (Vec3::random(-1.0, 1.0)).unit();
        }
            ...
            for dj in 0..2 {
                for dk in 0..2 {
                    c[di][dj][dk] = self.randvec[
                        (self.perm_x[(i.wrapping_add(di as i32) as usize) & 255] ^
                         self.perm_y[(j.wrapping_add(dj as i32) as usize) & 255] ^
                         self.perm_z[(k.wrapping_add(dk as i32) as usize) & 255]) as usize
                    ];
                }
            }
        }
